// use this syntax when my component fits on one line
const ListItem = props => <li className="list-item">{props.item.name}</li>

// use this when my component has no logic outside JSX
const List = ({ items }) => (
  <ul className="list">{items.map(item => <ListItem item={item} />)}</ul>
)

// use this when the component needs logic outside JSX.
const Body = props => {
